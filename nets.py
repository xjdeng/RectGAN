...
def build_discriminator( shape, build_disc=True ) :
    ...
    x = Conv2D( 64*factor2, (4, 4), strides=(2, 2),
        padding='same',
    ...
    if build_disc:
        ...
        # build encoder.
        x = Conv2D(Args.noise_shape[2], (5,5) , activation='tanh')(x)
        return models.Model( inputs=face, outputs=x )
...
def build_gen( shape ) :
    ...
    def deconv2d( x, filters, shape=(5, 5), mystrides = (2, 2)) :
        ...
        x= Conv2DTranspose( filters, shape, padding='same',
            strides=mystrides, kernel_initializer=Args.kernel_initializer )(x)
    ...
    x= Conv2DTranspose( 512*factor, (4, 4),
        kernel_initializer=Args.kernel_initializer )(x)
    x = BatchNormalization(momentum=Args.bn_momentum)( x )
    x = LeakyReLU(alpha=Args.alpha_G)( x )
    #x = ReLU()(x)
    x = deconv2d( x, 256*factor,mystrides=(1,1))
    x = deconv2d( x, 256*factor, mystrides = st[0])
    x = deconv2d( x, 128*factor, mystrides = st[1])
    x = deconv2d( x, 64*factor, mystrides = st[2])
    # 4x4
    x = deconv2d( x, 64*factor,mystrides=(1,1))
    #x = deconv2d( x, 256*factor2, mystrides = (4,2))
    # 8x8

    #x = deconv2d( x, 128*factor2, mystrides = (4,2) )
    # 16x16

    #x = deconv2d( x, 64*factor2 , mystrides = (2,2))
    # 32x32

    # Extra layer
    x = Conv2D( 64*factor, (3, 3), padding='same',
        kernel_initializer=Args.kernel_initializer )( x )
    x = BatchNormalization(momentum=Args.bn_momentum)( x )
    x = LeakyReLU(alpha=Args.alpha_G)( x )
    #x = ReLU()(x)
