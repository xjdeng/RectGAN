...
def build_discriminator_old( shape, build_disc=True ) :
    '''
    Build discriminator.
...
def build_discriminator( shape, build_disc=True ) :
    ...
    if build_disc:
        ...
        # build encoder.
        x = Conv2D(Args.noise_shape[2], (5,5) , activation='sigmoid')(x)
        return models.Model( inputs=face, outputs=x )



def build_gen_old( shape ) :
    ...
    x= Conv2DTranspose( 512*factor2, (4, 4),
        kernel_initializer=Args.kernel_initializer )(x)
    x = BatchNormalization(momentum=Args.bn_momentum)( x )
    x = LeakyReLU(alpha=Args.alpha_G)( x )
    ...
    # Extra layer
    x = Conv2D( 64*factor2, (3, 3), padding='same',
        kernel_initializer=Args.kernel_initializer )( x )
    x = BatchNormalization(momentum=Args.bn_momentum)( x )
    x = LeakyReLU(alpha=Args.alpha_G)( x )
    # 32x32
